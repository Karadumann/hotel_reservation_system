...
public class ManagerStatsController {
    ...
    @GetMapping("/receptionists-stats")
    public Map<String, Object> getReceptionistsStats() {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        User user = userService.findByUsername(auth.getName());
        List<User> receptionists = userService.getUsersByHotelIdAndRole(user.getHotel().getId(), "ROLE_RECEPTIONIST");

        Map<String, Long> receptionistsByRole = receptionists.stream()
                .collect(Collectors.groupingBy(receptionist -> receptionist.getRole().getRoleName(), Collectors.counting()));

        List<String> labels = new ArrayList<>(receptionistsByRole.keySet());
        List<Long> values = new ArrayList<>(receptionistsByRole.values());
